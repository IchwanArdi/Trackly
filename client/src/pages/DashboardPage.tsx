  ...
  return (
    <div className="space-y-5 sm:space-y-7">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Button id="btn-add-entry" variant="primary" size="md" icon={<Plus size={16} />} onClick={() => navigate('/log')} className="hidden sm:inline-flex">
          Log activity
        </Button>
      </div>
      ...
      <Card className="p-4! sm:p-5!">
        <div className="flex flex-col gap-4 mb-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-muted mt-0.5">{activeTab === 'calendar' ? 'Monthly tracking calendar' : activeTab === 'grid' ? 'Last 90 days grid' : 'Last 30 days trend'}</p>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2.5">
            ...
            <div className="flex bg-surface p-1 rounded-lg border border-border">
              <button
                ...
                className={`flex items-center gap-1 sm:gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md transition-all cursor-pointer ${
                  activeTab === 'calendar' ? 'bg-card text-foreground shadow-sm' : 'text-muted hover:text-foreground'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-1 sm:gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md transition-all cursor-pointer ${activeTab === 'grid' ? 'bg-card text-foreground shadow-sm' : 'text-muted hover:text-foreground'}`}
              >
                <Grid size={13} />
                Grid
              </button>
              <button
                ...
                className={`flex items-center gap-1 sm:gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md transition-all cursor-pointer ${activeTab === 'trend' ? 'bg-card text-foreground shadow-sm' : 'text-muted hover:text-foreground'}`}
              >
                <LineChart size={13} />
                Trend
              </button>
            ...
            <select
              ...
              className="text-xs bg-surface border border-border rounded-md px-2.5 py-1.5 text-foreground focus:outline-none focus:ring-1 focus:ring-accent cursor-pointer font-medium min-h-9"
            >
        ...
        {/* Conditional Content */}
        {activeTab === 'calendar' && <MonthlyCalendar entries={filteredEntriesForHeatmap} category={selectedCategoryObj} color={heatmapColor} />}

        {activeTab === 'grid' && <ContributionHeatmap entries={filteredEntriesForHeatmap} color={heatmapColor} />}
          ...
          <div className="pt-2">
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={trendData} margin={{ left: -20, right: 5, top: 10, bottom: 0 }}>
                ...
                <YAxis allowDecimals={false} tick={{ fontSize: 10, fill: 'var(--color-muted)' }} axisLine={false} tickLine={false} width={30} />
                <Tooltip
                  contentStyle={{
                  ...
                  formatter={(val: unknown) => [`${val} ${selectedCategoryFilter === 'all' ? 'entries' : (selectedCategoryObj?.unit ?? '')}`, selectedCategoryFilter === 'all' ? 'Volume' : 'Logged Amount']}
                />
                <Area type="monotone" dataKey="value" stroke={heatmapColor} strokeWidth={2} fillOpacity={1} fill="url(#trendGradient)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="p-4! sm:p-5!">
          ...
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={weeklyData} barSize={18}>
              ...
              <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: 'var(--color-muted)' }} axisLine={false} tickLine={false} width={20} />
              <Tooltip
                contentStyle={{
                  background: 'var(--color-card)',
                  border: '1px solid var(--color-border)',
        ...
        <Card className="p-4! sm:p-5!">
          <div className="flex items-center justify-between mb-3">
            ...
            <button id="btn-view-all-history" onClick={() => navigate('/history')} className="text-xs text-accent hover:underline min-h-9 flex items-center px-1">
              View all
            </button>
          </div>
            ...
            {recentEntries.map((entry) => {
              ...
              return (
                <div key={entry.id} className="flex items-center gap-3 py-2 border-b border-border last:border-0">
                  <div className="w-8 h-8 rounded-md flex items-center justify-center shrink-0" style={{ background: `${cat.color}1a` }}>
